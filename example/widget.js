...
const R = {
  ...
  time: (() => {
    // EXAMPLE:BEGIN use a fixed date for the example data file
    //const t = new Date();
    const t = new Date((new Date("2022-03-09")).getTime() - 1000 * 60 * 60 * 11.5);
    // EXAMPLE:END
...
async function getSeriesValues(series) {
  ...
  async function executeQueries(queries) {
    ...
    // EXAMPLE:BEGIN we load the data from the example data file
    /*
    ...
    */
    ...
    const exampleDataFile = fileManager.joinPath(fileManager.documentsDirectory(), 'data.json');
    const response = JSON.parse(fileManager.readString(exampleDataFile));
    // EXAMPLE:END 
